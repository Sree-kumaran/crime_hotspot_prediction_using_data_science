...
function Textarea({ label, helperText, error, required, className, ...props }) {
  const id =
    props.id ||
    `textarea-${label?.replace(/\s+/g, "-").toLowerCase() || "field"}`;
  ...
  return (
    <div className="space-y-1.5">
      ...
      <textarea
        ...
        className={cn(
          "w-full min-h-[100px] rounded-lg border bg-palette-ink px-3 py-2 text-sm text-palette-almond placeholder:text-palette-lilac/50",
          error
